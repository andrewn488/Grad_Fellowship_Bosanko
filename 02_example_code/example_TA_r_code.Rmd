...
**Comments**
    ...
    - cereal doesn't appear to sell in big bulk quantities (sorry, Costco!)
...
```{r}
wilcox.test(cereal_ps$price[cereal_ps$promo == 0], cereal_ps$price[cereal_ps$promo == 1], conf.int = TRUE) # running but not correct?
...
**Comments**
    ...
    is 1, showing a decrease in price of $1 (is that correct?)
    - Wilcox on Ad type A very statistically significant with an extremely small Pvalue, and the difference of the medians
    is only .74, showing a decrease in price of $.74 with ad type B (is that correct?)
    - Wilcox on Ad type B very statistically significant with an extremely small Pvalue, and the difference of the medians
    is .60, showing a decrease in price of $.60 with ad type A (is that correct?)

+ Advertisement and Promotions on Units sold

```{r}
wilcox.test(cereal_ps$units[cereal_ps$promo == 0], cereal_ps$units[cereal_ps$promo == 1], conf.int = TRUE)#running but not correct?
...
#super rough will need to fix up to see if they are producing the right interpretations of what I want
```
...
**Comments**
    ...
    - Wilcox on promo very statistically significant with an extremely small Pvalue, and the difference of the medians
    is almost 3, showing 3 more units sold with a promo(is that correct?)
    - Wilcox on Ad type A very statistically significant with an extremely small Pvalue, and the difference of the medians
    is almost 3, showing 3 more units sold with ad type B(is that correct?)
    - Wilcox on Ad type B very statistically significant with an extremely small Pvalue, and the difference of the medians
    is 2, showing 3 more units sold with ad type A(is that correct?)
...
**Comments**
    ...
    - Again, we see that it is statistically significant in the difference for promotions, and when we look at the count it is aparent that the median difference is 4 units, in the favor of a promotion,  compared to what we saw visually.(is that correct?)
    - We can see that with the 2 different advertisement options, A does sell the most units, but the small campaign out sells the medium sized campaign when it comes to Median.(is that correct?)
...
#should we be comparing stores?
#compares if average ad value  is different across the units
```
